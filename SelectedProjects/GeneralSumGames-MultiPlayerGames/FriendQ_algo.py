"""Friend Q Algo for 2 player Soccer game"""
import numpy as np
import random
...
AlgorithmName="Friend_Q"           # TODO: Make sure name is correct
...
_, actionDistribution=multiAgent.get_V_and_ActionDistribution(ds, get_CoordinatedEquilibrium)

# actionDistribution[0]=np.zeros(actionSpace[0])  # modify distribution at reset force A:'S' and B:'stand' actions
# actionDistribution[0, monitoractA_num]=1.0
# actionDistribution[1]=np.zeros(actionSpace[1])
# actionDistribution[1, monitoractB_num]=1.0
...
for iteration in range(End):
    ...
    Vsprime, actionDistribution=multiAgent.get_V_and_ActionDistribution(dsprime, get_CoordinatedEquilibrium) # get next state value and action distribution based on current Q table
...
# plt.semilogx(range(Qdiff.shape[0]), Qdiff)
plt.plot(Qdiff)
plt.ylim(-0.05, 0.5)
plt.yticks(np.linspace(-0.05,0.5, num=12))

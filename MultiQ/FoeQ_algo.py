"""Foe Q Algo for 2 player Soccer game"""
import numpy as np
import random
...
AlgorithmName="Foe_Q"           # TODO: Make sure name is correct
...
_, actionDistribution=multiAgent.get_V_and_ActionDistribution(ds, get_AdversarialEquilibrium, kwargs={'playerSlicings':playerSlicings})
...
for iteration in range(End):
    ...
    Vsprime, actionDistribution=multiAgent.get_V_and_ActionDistribution(dsprime, get_AdversarialEquilibrium, kwargs={'playerSlicings':playerSlicings}) # get next state value and action distribution based on current Q table
